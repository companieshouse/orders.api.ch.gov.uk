...
module "ecs-service" {
  ...
  multilb_listeners               = {
    ...
    "priv-api-lb": {
      listener_arn             = data.aws_lb_listener.service_lb_private_listener.arn,
      load_balancer_arn = data.aws_lb.service_lb_private.arn
    }
  }
  ...
  container_version = var.orders_api_version
